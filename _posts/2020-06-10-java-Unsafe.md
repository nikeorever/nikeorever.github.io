...
`Unsafe` 提供了一组比较低级（low-level）API允许Java程序员执行一些在Java层面上"不安全"的操作，比如说内存的分配和释放，"不安全"其实是相对的，如果一个Java程序员有一些C/C++开发经验的话，那这些"不安全"的API可以成为程序员窥探Java底层的工具。JDK中有大量的API借助了`Unsafe`：比如原子操作类（Atomic*）， `java.util.concurrent.locks.LockSupport`，支持并发操作的集合（`java.util.concurrent.ConcurrentHashMap`）等等。
